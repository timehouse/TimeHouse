package _08_fillinfo.controller;
...
@WebServlet(
		urlPatterns={"/FillInfo.controller"}
)
	...
	protected void doGet(HttpServletRequest request,
		...
		//驗證資料
				  ...
				  if(phone_number==null&&tel==null||phone_number.length()==0&&tel.length()==0)
				  {
					...
					"/01_Client/_08_FillInfo/FillInfo.jsp").forward(request, response);
					return;
					  }
		...
		//根據model執行結果顯示view		
				if("SelectId".equals(prodaction)){
					...
					request.getRequestDispatcher(
								"/01_Client/_08_FillInfo/FillInfoOut.jsp").forward(request, response);
					}
